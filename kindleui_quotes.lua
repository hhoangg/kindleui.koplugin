...
The entries below are lifted verbatim from simpleui.koplugin
(https://github.com/doctorhetfield-cmd/simpleui.koplugin), MIT licensed,
Copyright (c) 2026 doctorhetfield-cmd. Kept in their original
`{ q = ..., a = ..., b = ... }` shape so a future update there can be diffed
against this file rather than re-transcribed by hand.

They are all in English. The rest of this plugin's UI is not, so adding
Vietnamese entries is expected -- the upstream file invites exactly that
("Edit freely"), and nothing here depends on the language.

THE PICK IS STABLE FOR A CALENDAR DAY, DELIBERATELY

The lock screen redraws whenever the device wakes, which with a periodic clock
refresh can be every few minutes. Picking randomly would mean a different quote
every time the clock ticked over -- impossible to finish reading, and faintly
maddening. So the index comes from the date itself: same day, same quote, with
no state to store and nothing to keep in sync across a reboot.

The multiplier is a prime so consecutive days land far apart in the table. A
plain `day % n` would walk the list in order, which would show the Fiction
section for weeks and then nothing but Philosophy for weeks more.
]]
...
--- The quote for a given day; defaults to today.
-- `t` is an os.date("*t") table so a caller (or a test) can ask for a fixed day.
function Quotes.ofTheDay(t)
    t = t or os.date("*t")
    local n = #Quotes.LIST
    if n == 0 then return nil end
    local key = t.year * 10000 + t.month * 100 + t.day
    return Quotes.LIST[(key * 7919) % n + 1]
end
